...
- `ak_live_abc123xyz` - Public key ID
- `sk_live_def456uvw` - Secret key (keep this private!)
...
const [publicKey, secretKey] = API_KEY.split('.');
...
public_key, secret_key = API_KEY.split('.')
...
See [webhooks.mdx](./webhooks) for complete webhook integration guide.
...
- **Authentication**: [authentication](./authentication) - Deep dive into HMAC signature
- **Webhooks**: [webhooks](./webhooks) - Complete webhook integration guide
- **Payment Links**: [payment-links](./payment-links) - Guide to using payment links
- **Error Handling**: [errors](./errors) - Error codes and troubleshooting
- **Rate Limits**: [rate-limits](./rate-limits) - API rate limiting policies
