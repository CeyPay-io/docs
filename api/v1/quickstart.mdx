...
<CardGroup cols={3}>
  <Card title="Sandbox Account" icon="user-check">
    A CeyPay [sandbox account](https://sandbox.ceypay.io) — no KYC required to start building
  </Card>
  <Card title="KYC Verification" icon="id-card">
    Required only to go live at [merchant.ceypay.io](https://merchant.ceypay.io)
  </Card>
...
<CardGroup cols={1}>
  ...
  <Card title="Sandbox" icon="flask" href="/integrations/sandbox">
...
<Steps>
  <Step title="Generate API Key">
    Keys are issued per environment and are **not** interchangeable. This guide uses sandbox, so generate a sandbox key.

    1. Log in to the [Sandbox Dashboard](https://sandbox.ceypay.io) — or [merchant.ceypay.io](https://merchant.ceypay.io) when you are ready for production
    2. Navigate to **Settings** → **API Keys**
    3. Click **Generate New API Key**
    4. Give it a descriptive name (e.g., "Sandbox API Key")
    ...
    **Important**: Store your API key securely (environment variable, secrets manager, etc.)
    ...
    <Note>
      A sandbox key will not authenticate against production, and a production key will not authenticate against sandbox. See the [Sandbox guide](/integrations/sandbox) for the full comparison.
    </Note>
  ...
  <Step title="Make Your First API Call">
    ...
    <Note>
      Every example below targets **sandbox** (`https://sandbox-api.ceypay.io`). To go live, swap the base URL to `https://api.ceypay.io` and use a production key — nothing else changes.
    </Note>
...
async function createPayment() {
  ...
  const response = await axios.post(
    `https://sandbox-api.ceypay.io${path}`,
    JSON.parse(body),
    {
      headers: {
...
def create_payment():
    ...
    response = requests.post(
        f'https://sandbox-api.ceypay.io{path}',
        json=body_dict,
        headers={
...
# Your API key
API_KEY="ak_live_d86855bbd8651fcb5f469203621c88e3.sk_live_6985fca378cd894dee359813bd6bb12f4a2922bc81f023f0d062631e3190ff3e"

# Split into key ID and secret
KEY_ID="${API_KEY%%.*}"
SECRET="${API_KEY#*.}"

# Derive signing key from secret (matches PHP: hash('sha256', $secret))
...
BODY='{"amount":12,"currency":"USDT","provider":"BYBIT","goods":[{"name":"Wireless Headphones","description":"Noise-cancelling Bluetooth headphones","mccCode":"5732"}],"webhookUrl":"https://mystore.com/api/payment-webhook","customerBilling":{"email":"customer@example.com","firstName":"John","lastName":"Doe","phone":"+94771234567","address":"123 Main Street"}}'
...
curl -X POST "https://sandbox-api.ceypay.io${API_PATH}" \
...
```
    ...
    See [webhooks.md](/api/v1/webhooks) for complete webhook integration guide.
...
curl -X POST https://sandbox-api.ceypay.io/v1/webhooks/test \
  -H "x-api-key: $KEY_ID" \
  -H "x-timestamp: $TIMESTAMP" \
  -H "x-signature: $SIGNATURE" \
...
<CardGroup cols={2}>
  ...
  <Card title="Payment Links" icon="link" href="/api/v1/reference/create-payment-link">
...
Full endpoint reference, with request and response schemas and an interactive playground, is in the [API Reference](/api/v1/reference/create-payment).
