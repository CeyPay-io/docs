...
| `scenarioId`           | string (UUID) | Yes      | Scenario ID from the `scenario-code/list` endpoint                                       |
...
| `status`          | string        | No       | Filter by status: `INITIAL`, `CONTRACT_SIGNED`, `CONTRACT_TERMINATED`, `CONTRACT_EXPIRED` |
...
Terminates a signed Direct Debit contract. Only contracts in `CONTRACT_SIGNED` status can be terminated.
...
| Status | Description                                         |
| ------ | --------------------------------------------------- |
| 400    | Bad request - contract not in `CONTRACT_SIGNED` status |
| 401    | Unauthorized - invalid HMAC signature               |
| 404    | Contract not found                                  |
| 429    | Too many requests - rate limit exceeded             |
...
<Note>
  Direct debit payments do not include `qrContent` or `checkoutLink` fields — the payment is charged directly without user interaction. Payment starts in `INITIATED` status and transitions to `PAID` via webhook notification. In sandbox mode, payments are automatically marked as `PAID`.
</Note>
...
| Status                | Description                                        |
| --------------------- | -------------------------------------------------- |
| `INITIAL`             | Contract created but not yet signed by user        |
| `CONTRACT_SIGNED`     | User has signed the contract (ready for payments)  |
| `CONTRACT_TERMINATED` | Contract has been terminated                       |
...
See [Webhooks Guide](./webhooks.md) for payload format, signature verification, and retry policy.
...
See [Error Codes](./errors.md) for detailed error handling.
...
- [Authentication](./authentication.md) - HMAC signature details
- [Webhooks](./webhooks.md) - Receive payment notifications
- [Rate Limits](./rate-limits.md) - API rate limiting
- [Errors](./errors.md) - Error codes reference
