...
- **Attribution**: Since this is a programmatic request, the withdrawal is attributed to your merchant account's `ADMIN` user. At least one `ADMIN` user must exist on the merchant account, or the request will be rejected with `403`.
...
| 403    | Forbidden - Merchant account is inactive, not approved, or has no ADMIN user to attribute the request to.                |
