...
| `allow_quantity_buy`  | boolean | No       | Allow customers to purchase multiple quantities. Default: `false`.             |
| `max_quantity`        | number  | No\*     | Maximum quantity allowed per purchase. Required if `allow_quantity_buy` is true. |
...
| `allow_quantity_buy`  | boolean | Allow quantity purchase.  |
| `max_quantity`        | number  | Max quantity per purchase.|
